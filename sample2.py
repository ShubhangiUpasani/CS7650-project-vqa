...
model.load_state_dict(torch.load("output2/model.pt"))
...
valloader = torch.utils.data.DataLoader(valset, batch_size = 3000, shuffle = True, num_workers = 2)

criterion = nn.CrossEntropyLoss()
optimizer = optim.RMSprop(model.parameters(), lr=9e-5, weight_decay=1e-4, momentum=0.99)
...
for epoch in range(250):  # loop over the dataset multiple times
    ...
    with open("output2/output.txt","a") as f:
    ...
    with open("output2/train_loss.txt","a") as f:
        f.write(str(total_loss))
        f.write("\n")
    f.close()
   
    with open("output2/val_loss.txt","a") as f:
        f.write(str(total_val_loss))
        f.write("\n")
    f.close()
    torch.save(model.state_dict(),"output2/model.pt")
