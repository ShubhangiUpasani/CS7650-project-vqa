...
optimizer = optim.RMSprop(model.parameters(), lr=1e-4, weight_decay=1e-6, momentum=0.99)
...
for epoch in range(256):  # loop over the dataset multiple times
    ...
    with open("output1/output.txt","a") as f:
    ...
    with open("output1/train_loss.txt","a") as f:
        f.write(str(total_loss))
        f.write("\n")
    f.close()
   
    with open("output1/val_loss.txt","a") as f:
